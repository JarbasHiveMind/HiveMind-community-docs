...
- [the hivemind is a mycroft add-on](#the-hivemind-is-a-mycroft-add-on)
- [the hivemind connects devices](#the-hivemind-connects-devices)
- [the hivemind decentralizes mycroft-core](#the-hivemind-decentralizes-mycroft-core)
...
- [the hivemind safely exposes mycroft to the web](#the-hivemind-safely-exposes-mycroft-to-the-web)
- [the hivemind isolates the messagebus](#the-hivemind-isolates-the-messagebus)
- [the hivemind is a protocol that transparently integrates with mycroft](#the-hivemind-is-a-protocol-that-transparently-integrates-with-mycroft)
- [the hivemind provides skill isolation](#the-hivemind-provides-skill-isolation)
- [the hivemind can be used to integrate mycroft with any platform](#the-hivemind-can-be-used-to-integrate-mycroft-with-any-platform)
...
- step 1: install [mycroft-core](https://github.com/MycroftAI/mycroft-core) / own a [mycroft device](https://www.kickstarter.com/projects/aiforeveryone/mycroft-mark-ii-the-open-voice-assistant) / install a [project shipping mycroft](https://github.com/ChanceNCounter/awesome-mycroft-community#distributions-images-projects-shipping-mycroft)
...
- step 1: install the [cli terminal](https://github.com/OpenJarbas/HiveMind-cli) and [register it](https://github.com/JarbasHiveMind/HiveMind-core/blob/dev/examples/add_keys.py) (**see below**) in the mycroft node 
...
I have seen many people host mycroft in the cloud, but the messagebus in unencrypted and doesn't even [support ssl](https://github.com/MycroftAI/mycroft-core/pull/1148), a common solution is to setup nginx and/or [smartgic docker image](https://github.com/smartgic/docker-mycroft)
...
the message bus is [a privacy nightmare and should be closed from the outside world](https://github.com/Nhoya/MycroftAI-RCE), anyone can connect to it and capture every single thing going on in "mycroft's nervous system" or inject new commands

- step 1: install [ovos_utils](https://github.com/OpenVoiceOS/ovos_utils) on your *mycroft node* (or get it's ip address)
...
- step 2: get the ip address of your *mycroft node* and set the url as `ws://{ip_address}:8181` 
- step 3: :x: (disable the firewall in mycroft device)
- step 4: :question: 
...
this is solved by including proper metadata in the bus message.context, you can read more about how mycroft-core routes messages internally in [the wiki](https://github.com/JarbasHiveMind/HiveMind-core/wiki/(mycroft-core)-message-routing). each connected client only receives what it should, not everything
...
- step 1: install the [android app](https://github.com/MycroftAI/Mycroft-Android)
- step 2: get the ip address of your *mycroft node* and set the url as `wss://{ip_address}:5678?authorization={encoded}` 
    ...
    - TODO need a flag in hivemind-core to support this (assume all messages are payloads of [type "bus"](https://github.com/JarbasHiveMind/HiveMind-core/wiki/Messages#mycroft-messages)) per client (?)
- step 3 :x: **NOT YET**!
...
- step 3: :tada:
- step 4: :x: a skill changed mycroft-core at runtime (?)
...
- step 2: as soon as mycroft loads this skill everything stops workings, [related issue](https://github.com/MycroftAI/mycroft-core/issues/2905)
...
- :exclamation: **LocalHive replaces mycroft-core**
...
- step 3 :tada: **shared mycroft with friends**
...
- step 3 :tada: **mycroft is a productivity tool**
...
- step 3 :tada: **control mycroft from your phone**
...
- step 3 :tada: **mycroft is a chatbot**
...
- step 1: replace all mycroft bus connections with [hivemind-bus-client](https://github.com/JarbasHiveMind/hivemind_websocket_client/) (python) or [HiveMindJS](https://github.com/JarbasHiveMind/HiveMind-js) (javascript)
