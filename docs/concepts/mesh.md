...
`CASCADE` floods in all directions like `PROPAGATE`, and every node that can answer sends a response back. The originator collects responses via `CascadeAggregator` and applies a `cascade_select_callback` to pick the best answer. See [Protocol — CASCADE](protocol.md).
