...
If a node needs to securely communicate or authenticate another (that isn't the master) it can do so via the public key. See the section for [intercom messages](./04_protocol.md#intercom-message) for more details

Groups of devices can also be targeted via their `site_id`, for example, we can [propagate](./04_protocol.md#propagate-message) a speak message to announce dinner is ready or [broadcast](./04_protocol.md#broadcast-message) a bus message to order all devices in a certain area equipped with a camera to take a picture. 
