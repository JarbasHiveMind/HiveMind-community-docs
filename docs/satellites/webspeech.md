...
!!! abstract "In a nutshell"
    ...
    - By default it ships audio as base64 over the bus (`recognizer_loop:b64_audio`) — the server needs `ovos-dinkum-listener >= 0.3.0a1` and a one-time `hivemind-core allow-msg "recognizer_loop:b64_audio"`. Switching the audio transport to `binary` instead sends a WIRE-1 `STT_AUDIO_HANDLE` frame, admitted through hivemind-core's binary policy rather than the bus allow-list.
...
1. Run a listener new enough to decode that message — `ovos-dinkum-listener >= 0.3.0a1`.
